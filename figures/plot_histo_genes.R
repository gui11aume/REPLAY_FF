...
cutoff = quantile(exprs$ES2, 0.6, na.rm=TRUE)
...
dattest_out = subset(dat, V6 == "test" & is_in_a_gene & !is_in_a_hi_gene)

# Mean-aggregate scores per barcode.
test_in = aggregate(dattest_in$V2, FUN=mean,
   by=list(brcd=dattest_in$V1, MM=dattest_in$V3,
           rep=dattest_in$V7, GC=dattest_in$V9))

test_out = aggregate(dattest_out$V2, FUN=mean,
